...
private Batch filterBatch(Batch batch){
   ...
   List<Batch> batchesInPrep= batchMgr.findBatchByState(BatchState.InPreparation);
    Batch batch2=null;
   if(batchesInPrep.size()>0){
     ...
     batch2  =batchesInPrep.get(0);
       ...
       batch1.setBatchState(BatchState.InPreparation);
       batchMgr.saveOrUpdate(batch2);
