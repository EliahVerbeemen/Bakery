...
public class PurchaseOrderDeserializer extends JsonDeserializer<PurchaseOrder> {
    ...
    public PurchaseOrder deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JacksonException {
     ...
     /*   items.forEach(i->{
              ...
              String productNumber=  i.get("ProductNumber").asText();
              String productName=  i.get("productName").asText();
                int quantity=  i.get("quantity").asInt();
                String specialInstruction=  i.get("specialInstruction").asText();
                ...
                purchaseProduct.setProductName(productName);
